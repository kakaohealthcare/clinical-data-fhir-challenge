...
과제 풀이 시간을 아끼기 위한 요약본입니다. **아래 리소스가 모두 이번 과제에 필요한 것은 아닙니다** — 어떤 리소스를 쓸지는 여러분이 판단하세요.  
정확한 정의는 공식 문서(<https://hl7.org/fhir/R4/resourcelist.html>)가 기준입니다.
...
- **<u>식별자·로컬 코드의 `system` URI는 네임스페이스 역할일 뿐 실제 접속 가능할 필요가 없습니다.<br>표준 코드체계가 아닌 병원 내부 값이라면 임의의 URI(예: `https://example.org/...`)를 정해 일관되게 사용하면 됩니다.</u>**
